...
<Policy kerberos>
  ...
  <Limit All>
    ...
    services.udev.extraRules = ''
      # Enhanced USB Printer Auto-Detection Rules
      
      # Zebra printer specific rules (vendor ID 0a5f)
      SUBSYSTEM=="usb", ATTR{idVendor}=="0a5f", ATTR{idProduct}=="00d7", MODE="0666", GROUP="lp", TAG+="systemd", ENV{SYSTEMD_WANTS}="auto-printer-setup@$env{BUSNUM}-$env{DEVNUM}.service"
      SUBSYSTEM=="usb", ATTR{idVendor}=="0a5f", ATTR{idProduct}=="00d8", MODE="0666", GROUP="lp", TAG+="systemd", ENV{SYSTEMD_WANTS}="auto-printer-setup@$env{BUSNUM}-$env{DEVNUM}.service"
      SUBSYSTEM=="usb", ATTR{idVendor}=="0a5f", ATTR{idProduct}=="00d9", MODE="0666", GROUP="lp", TAG+="systemd", ENV{SYSTEMD_WANTS}="auto-printer-setup@$env{BUSNUM}-$env{DEVNUM}.service"
      
      # Generic USB printer detection (USB printer class)
      SUBSYSTEM=="usb", ATTR{bInterfaceClass}=="07", MODE="0666", GROUP="lp", TAG+="systemd", ENV{SYSTEMD_WANTS}="auto-printer-setup@$env{BUSNUM}-$env{DEVNUM}.service"
      
      # USB serial devices that might be printers
      SUBSYSTEM=="usb", ATTR{bInterfaceClass}=="02", MODE="0666", GROUP="lp", TAG+="systemd", ENV{SYSTEMD_WANTS}="auto-printer-setup@$env{BUSNUM}-$env{DEVNUM}.service"
    '';

    # Simplified automatic printer detection and setup service
    systemd.services."auto-printer-setup@" = {
      description = "Automatic USB printer detection and configuration";
      serviceConfig = {
        Type = "oneshot";
        ExecStart = "${pkgs.writeShellScript "auto-printer-setup.sh" ''
          #!/bin/bash
          exec 1>&2  # Redirect stdout to stderr so it shows in journal
          
          echo "=== Auto printer setup started ==="
          sleep 3
          
          # Ensure CUPS is running
          if ! systemctl is-active --quiet cups; then
            echo "CUPS not running, starting..."
          ...
          # Find and configure USB printers
          lpinfo -v | grep usb:// | while read uri; do
            read -r -a parts <<< "\$uri"
            printerUri="\$parts[1]"
            echo "Found USB printer: \$printerUri"
            
            # Use the URI as the printer name
            # Extract a more readable name from the URI by removing the protocol prefix and special characters
    ...
    # Boot-time printer detection service
    systemd.services.printer-setup-boot = {
      description = "Detect and configure USB printers at boot";
      wantedBy = [ "multi-user.target" ];
      after = [ "cups.service" ];
      serviceConfig = {
        Type = "oneshot";
        ExecStart = "${pkgs.writeShellScript "printer-setup-boot.sh" ''
          #!/bin/bash
          exec 1>&2  # Redirect stdout to stderr so it shows in journal
          
          echo "=== Boot printer setup started ==="
          sleep 5
          
          # Ensure CUPS is running
          if ! systemctl is-active --quiet cups; then
            echo "CUPS not running at boot, starting..."
            systemctl start cups
            sleep 3
          fi
          
          echo "Scanning for USB printers at boot..."
          
          # Find and configure USB printers
          lpinfo -v | grep usb:// | while read uri; do
            read -r -a parts <<< "\$uri"
            printerUri="\$parts[1]"
            echo "Found USB printer at boot: \$printerUri"
            ...
            if ! lpstat -p "\$name"; then
              echo "Adding printer at boot: \$name"
              if lpadmin -p "\$name" -E -v "\$printerUri" -m raw; then
                cupsenable "\$name" || echo "Failed to enable \$name"
                cupsaccept "\$name" || echo "Failed to accept jobs for \$name"
                echo "Successfully added printer at boot: \$name"
              else
                echo "Failed to add printer at boot: \$name"
              fi
            else
              echo "Printer \$name already exists at boot"
            fi
          done
          
          echo "=== Boot printer setup completed ==="
        ''}";
        RemainAfterExit = true;
        StandardOutput = "journal";
        StandardError = "journal";
